...
def reset_all_sessions():
    ...
    # 2. Clear Global Session Results
    global session_results
    session_results = {"brain": None, "chest": None}
    
    # 3. Return 'None' or empty values for all UI components to clear them
    # Order must match the 'outputs' list in the button click event
    return (
        None,           # img_in (gr.Image)
        "Brain MRI",    # mod_in (gr.Radio - reset to default)
        None,           # pdf_in (gr.File)
        [],             # chatbot (gr.Chatbot)
        "",             # msg_input (gr.Textbox)
        "Session Reset Complete" # status_box (gr.Textbox)
    )
...
def add_user_message(history, message):
    ...
    # Update: Use dictionary format to satisfy the "messages format" requirement
...
def bot_response(history):
    ...
    # Update status box or print to console
    print(stats)
...
with gr.Blocks(theme=gr.themes.Soft(), title="MedAssist AI") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            # We omit the 'type' argument here; Gradio will use its default based on your version
